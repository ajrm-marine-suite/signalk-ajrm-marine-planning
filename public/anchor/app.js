...
async function renderAbout() {
  ...
  try {
    const response = await fetch("/plugins/signalk-ajrm-marine-planning/anchor/version");
...
document.getElementById("refreshTideData").addEventListener("click", async () => {
  try {
    ...
    const response = await fetch("/plugins/signalk-ajrm-marine-planning/anchor/tide-data/refresh", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        stationName: document.getElementById("tideDataStationName").value.trim(),
...
async function useLiveSignalKInputs() {
  const status = document.getElementById("liveSignalKStatus");
  try {
    status.textContent = "Reading Signal K…";
    const response = await fetch("/plugins/signalk-ajrm-marine-planning/anchor/live");
    if (!response.ok) throw new Error(`server returned ${response.status}`);
    const live = await response.json();
    const numberOrNull = (value) => value == null || value === "" ? null : Number(value);
    const windMps = numberOrNull(live.windSpeedTrueMps ?? live.windSpeedApparentMps);
    const currentMps = numberOrNull(live.currentSpeedMps);
    const belowKeelM = numberOrNull(live.depthBelowKeelM);
    const belowSurfaceM = numberOrNull(live.depthBelowSurfaceM);
    const used = [];
    if (Number.isFinite(windMps)) { document.getElementById("windSpeed").value = (windMps / 0.514444).toFixed(1); used.push("wind"); }
    if (Number.isFinite(currentMps)) { document.getElementById("tidalStream").value = (currentMps / 0.514444).toFixed(1); used.push("current"); }
    if (Number.isFinite(belowKeelM)) { document.getElementById("echoBelowKeel").value = belowKeelM.toFixed(1); used.push("depth below keel"); }
    else if (Number.isFinite(belowSurfaceM)) { document.getElementById("echoBelowKeel").value = belowSurfaceM.toFixed(1); used.push("depth below surface"); }
    renderAll();
    status.textContent = used.length ? `Loaded ${used.join(", ")}. Review before use.` : "No usable live wind, current or depth values are available; manual values retained.";
  } catch (error) {
    status.textContent = `Live values unavailable (${error.message}); manual values retained.`;
  }
}

document.getElementById("useLiveSignalK")?.addEventListener("click", useLiveSignalKInputs);
